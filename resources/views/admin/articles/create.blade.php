<x-app-layout>
    <x-slot name="header">
        <div class="bg-gradient-to-r from-indigo-500 to-purple-500 p-6 rounded-3xl shadow-lg mt-4 border-b-4 border-indigo-300 flex justify-between items-center">
            ...
            <a href="{{ route('admin.articles.index') }}" class="bg-white text-indigo-600 hover:bg-indigo-50 transition rounded-xl shadow p-3 flex items-center justify-center" aria-label="Kembali ke Kelola Artikel">
            ...
            <div class="bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
                <form method="POST" action="{{ route('admin.articles.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="title" required class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition" placeholder="Masukkan judul mading yang menarik...">
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6" x-data="{ 
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="datetime-local" name="event_date" value="{{ old('event_date') }}" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition">
                            </div>
                            <div x-show="isEvent()" x-cloak x-transition>
                                <label class="block text-gray-700 font-bold mb-2">Lokasi Pelaksanaan</label>
                                <input type="text" name="event_location" value="{{ old('event_location') }}" placeholder="Contoh: Gedung Serba Guna / Link Zoom" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition">
                            </div>
                            ...
                            <div class="pt-2">
                                ...
                                <input type="datetime-local" name="expires_at" value="{{ old('expires_at') }}" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-red-400 focus:border-red-400 transition">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="content" rows="8" required class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition" placeholder="Tuliskan isi pengumuman mading secara detail di sini..."></textarea>
                    </div>

                    <div class="mb-8 bg-indigo-50 p-4 rounded-xl border border-indigo-200">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_pinned" value="1" class="rounded text-indigo-600 focus:ring-indigo-400 border-gray-300 w-5 h-5 shadow-sm">
